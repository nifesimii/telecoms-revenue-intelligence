...
Knowledge Base (fbb_commission_kb.md — loaded into system prompt)
        ↓
Finance Agent (Claude claude-sonnet-4-20250514 with tool use)
        ↓
...
**agent.py owns the Claude conversation loop only.**
It does not know about SQL or Presto.

**tool_executor.py owns the translation layer only.**
It receives a tool call from Claude and routes to queries.py.
It does not know about Claude's API format.
...
- Note: Claude classifies root cause from KB knowledge.
...
- Claude must use KB knowledge to explain query results
- Claude must never invent rules not documented in the KB
- When explaining zero-commission records, Claude must use
